			...
			<div className="glass-panel p-6 space-y-4 border-[var(--accent-cyan)] border">
					...
					<div>
						...
						<input
							...
							onChange={(e) =>
								setProfile({
									...profile,
									portfolioRepo: e.target.value,
								})
							}
						/>
						<p className="text-xs text-[var(--text-secondary)] mt-1">
							The GitHub repository where the bot will submit PRs
							for your portfolio updates.
						</p>
					</div>
				</div>
				<div className="flex justify-end gap-2 text-sm">
					<button
						...
						className="px-4 py-2 hover:text-black text-[var(--text-secondary)]"
					>
						Cancel
					</button>
					<button
						onClick={handleSave}
						disabled={loading}
						className="px-4 py-2 bg-black text-white rounded font-medium hover:bg-black/80"
					>
