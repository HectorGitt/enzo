						...
						parameters: {
							...
							properties: {
								repoName: {
									type: SchemaType.STRING,
									description:
										"Name of the repository or project (e.g. 'enzo', 'react-app')",
								},
							},
							required: ["repoName"],
						},
						...
						parameters: {
							...
							properties: {
								query: {
									type: SchemaType.STRING,
									description:
										"The topic or concept to search for",
								},
							},
							required: ["query"],
						},
	...
	try {
		...
		if (call) {
			...
			if (call.name === "search_by_date") {
				...
				console.log(
					`Total Wins: ${profile.wins.length}, Total Raw: ${profile.rawActivities?.length || 0}`,
				);
				...
				console.log(
					`Found ${relevantItems.length} matches (${wins.length} wins, ${raw.length} raw).`,
				);
				...
				const raw = (profile.rawActivities || []).filter((r) => {
					...
					return (
						rRepo.includes(repo) ||
						r.title.toLowerCase().includes(repo)
					);
				});
				...
				const scored = winsWithEmbeddings.wins.map((w) => ({
					item: w,
					score: w.embedding
						? cosineSimilarity(queryEmbedding, w.embedding)
						: 0,
				}));
			...
			tokensUsed +=
				finalResult.response.usageMetadata?.totalTokenCount || 0;
		} else {
...
async function ensureEmbeddings(
	wins: Win[],
): Promise<{ wins: Win[]; updated: boolean }> {
	let updated = false;
	const processingWins = [...wins];

	// Filter wins needing embeddings
	const needingEmbedding = processingWins.filter(
		(w) => !w.embedding || w.embedding.length === 0,
	);
			...
			await Promise.all(
				batch.map(async (w) => {
					try {
						...
						console.warn(
							`Failed to generate embedding for win ${w.id}`,
							e,
						);
					}
				}),
